...
- **Not seeded, with reasons.** `certificate-decoder` (async WebCrypto verification and expiry prose that goes stale daily), `jq-playground` (the answer needs the WASM binary at build time, and the Vite `?url` import is client-only), `uuid-ulid-generator` (v4 and ULID are random by definition — only the fixed-UUID inspector could ever be seeded), plus `logql-promql`, `cve-converter`, `url-codec` and `terraform-plan-summarizer`, whose builders are still trapped inside `init()`. Seeding the last four is mechanical, not blocked.
