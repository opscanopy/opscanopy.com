...
function readList(
  ...
  maxChanges: number,
  diagnostics: Diagnostic[],
): { list: ResourceChange[]; truncated: boolean; skipped: number } {
  if (!Array.isArray(raw)) return { list: [], truncated: false, skipped: 0 };
  ...
  for (const entry of raw) {
    ...
    list.push(readOne(entry, diagnostics));
  }
  return { list, truncated, skipped };
}

function readOutputs(raw: unknown): OutputChange[] {
...
export function parsePlanJson(doc: Record<string, unknown>, maxChanges: number): JsonParseResult {
  ...
  if (hasResourceChanges && !Array.isArray(doc.resource_changes)) {
    ...
    return {
      changes: [],
      drift: [],
      outputChanges: readOutputs(doc.output_changes),
      formatVersion,
