/**
 * JWT Decoder & Encoder — bundled examples for the picker. Each spans a useful
 * case: the canonical HS256 sample (its matching secret rides along so
 * verification lights up in one click), an ES256 token from RFC 7515 §A.3
 * (verifies with the bundled public JWK), a token whose `exp` is in the past
 * (renders an "expired" row), a token rich in registered claims, and an
 * unsecured `alg:"none"` token that raises a security warning.
 */
