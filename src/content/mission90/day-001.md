...
description: "Learn what DevOps actually is in under an hour: the code-to-production loop plus your first 6 terminal commands. Free, hands-on, with the real errors I hit."
phase: 1
minutes: 50
goals:
...
interviewQA:
  ...
  - q: "What is the difference between a shell and a terminal?"
    ...
    track: both
check:
  - q: "According to today's lesson, what is the core idea of DevOps?"
    options:
      - "A job title responsible for writing deployment scripts"
      - "Shortening the loop between writing code and running it reliably in production"
      - "A certification that replaces the need for a QA team"
      - "Using cloud servers instead of on-premises hardware"
    answerIndex: 1
    explain: "DevOps is a way of working, not a title or a tool — it's about collapsing the wall between writing code and running it."
  - q: "When you run an external command like `ls`, what does the shell actually do?"
    options:
      - "It executes the command directly inside the terminal's own process"
      - "It searches PATH for the executable and asks the kernel to start it as a new process"
      - "It sends the command to a remote CI server for execution"
      - "It only works if the command is a built-in like `cd`"
    answerIndex: 1
    explain: "The shell finds the executable via PATH and asks the kernel to start it as a new process — the terminal just draws the window."
goDeeperMinutes: 30
