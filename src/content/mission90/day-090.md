...
Tonight it converges. At 00:14 a single security-group change revoked tcp/443 ingress on `sg-web` — a fat-finger meant for `sg-bastion`. The load-balancer health checks to the web tier went red, so Route 53 failed traffic over to the passive region. And in that passive region the web pods are in `CrashLoopBackOff` because the `db-credentials` Secret was deleted. One change, three symptoms, all four layers you studied — DNS, AWS networking, Kubernetes, and containers — alight at the same moment.
...
- **Read the failover-cluster events** — `cat ~/k8s/events.txt`. The passive-region pods are `CrashLoopBackOff` with `FailedMount ... secret "db-credentials" not found`. The Secret was deleted; that's why the failover region is degraded too (Phase 4's Secrets).
