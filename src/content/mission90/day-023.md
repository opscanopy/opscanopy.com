...
check:
  - q: "What is the key difference between a Docker image and a container?"
    options:
      - "An image is a read-only template; a container is a running instance of it with a writable layer"
      - "An image only runs on Linux; a container runs on any OS"
      - "A container is stored in a registry; an image only exists locally"
      - "There's no real difference — the terms are interchangeable"
    answerIndex: 0
    explain: "Like a class and its objects: one image, many disposable containers. Deleting a container leaves the image untouched."
  - q: "Why shouldn't you rely on the `:latest` tag in production?"
    options:
      - "`:latest` images are always slower to pull than versioned ones"
      - "Docker Hub deprecates `:latest` tags after 90 days"
      - "`:latest` points at whatever the publisher tagged most recently, so it can silently change between pulls"
      - "`:latest` images cannot be used with `docker exec`"
    answerIndex: 2
    explain: "Two machines pulling the same `:latest` tag weeks apart can get different images — pin an explicit tag or digest instead."
goDeeperMinutes: 30
---
