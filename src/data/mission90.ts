...
export const days: Mission90Day[] = [
  ...
  d(50, 'vpc-1', 'VPC 1 — subnets, route tables, gateways', 55, { live: true }),
  d(51, 'vpc-2', 'VPC 2 — NAT, public vs private, bastion patterns', 55, { live: true }),
  d(52, 's3', 'S3 — buckets, policies, static hosting, lifecycle', 50, { live: true }),
  d(53, 'rds-managed-databases', 'RDS & managed databases', 50, { live: true }),
  d(54, 'load-balancers-auto-scaling', 'Load balancers & auto scaling', 55, { live: true }),
  d(55, 'route-53-tls-acm', 'Route 53 & TLS with ACM', 50, { live: true }),
  d(56, 'week-8-review-database-recovery', 'Week 8 review + Database Recovery', 45, { missionId: 'week8-database-recovery', live: true }),
  d(57, 'cloudwatch', 'CloudWatch — metrics, logs, alarms', 50, { live: true }),
  d(58, 'aws-cli', 'AWS CLI & scripting cloud operations', 50, { live: true }),
  ...
  d(66, 'why-kubernetes', 'Why Kubernetes — the problems it solves', 45, { live: true }),
  d(67, 'k8s-architecture-kubectl', 'K8s architecture + kubectl with kind on WSL2', 55, { live: true }),
  d(68, 'pods-deployments', 'Pods & Deployments', 55, { live: true }),
  d(69, 'k8s-services-networking', 'Services & networking in K8s', 55, { live: true }),
  d(70, 'configmaps-secrets', 'ConfigMaps & Secrets', 50, { live: true }),
  d(71, 'probes-resource-limits', 'Health probes & resource requests/limits', 50, { live: true }),
  d(72, 'debugging-workloads', 'Debugging workloads — describe, logs, events, crashloops', 55, { live: true }),
  d(73, 'week-11-review-kubernetes-chaos', 'Week 11 review + Kubernetes Chaos', 45, { missionId: 'week11-kubernetes-chaos', live: true }),
  d(74, 'ingress', 'Ingress & exposing apps properly', 55, { live: true }),
  d(75, 'helm', 'Helm — charts, values, releases (Go Deeper: GitOps with Argo CD)', 55, { live: true }),
  d(76, 'terraform-first-apply', 'IaC concepts + Terraform first apply', 50, { live: true }),
  d(77, 'terraform-variables-state', 'Terraform 2 — variables, outputs, state', 55, { live: true }),
...
export const missions: Mission90Mission[] = [
  ...
  },
  {
    ...
    blurb: 'One pod is stuck starting, the rest are CrashLoopBackOff — read the events and restore order.',
    unlockAfterDay: 73,
