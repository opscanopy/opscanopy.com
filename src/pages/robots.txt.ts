...
export function GET(): Response {
  const lines: string[] = [
    ...
    'Allow: /',
    '',
    '# Nothing is disallowed, deliberately. /search (all 5 locales) and /offline',
    '# are utility routes with no standalone search value — but they already',
    '# serve <meta name="robots" content="noindex,nofollow">, and a crawler has',
    '# to FETCH a page to see that. Disallowing them here suppressed the very',
    '# directive we want obeyed, which is how a blocked URL still ends up',
    '# indexed URL-only. Keeping them crawlable is what gets them dropped.',
    '#',
    '# The other two exclusions are independent and still in force: the sitemap',
    '# filter in astro.config.mjs, and the Pagefind opt-out via Shell\'s',
    '# searchIndex prop. Note nothing here backstops a page that loses its',
    '# noindex — Shell\'s noindex prop is now the only gate.',
    '',
