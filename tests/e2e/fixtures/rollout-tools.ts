/**
 * Fixtures for the ten tools added by the 2026-07 rollout.
 *
 * One module per batch, composed by ../tools.fixtures.ts. Batch modules exist so
 * that parallel agents can each own a file outright: every merge conflict in this
 * project's history came from several builders appending to one shared array.
 * Field rules live in ../tools.fixtures.ts — read that doc comment first.
 */
import { DEMO_LEAF_XSS_SUBJECT } from '../../../src/lib/cert-chain/fixtures';
import type { ToolFixture } from '../tools.fixtures';

export const ROLLOUT_FIXTURES: ToolFixture[] = [
  {
